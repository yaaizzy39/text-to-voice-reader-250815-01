...
### 📥 ダウンロード機能
- **音声ファイルダウンロード**: 生成した音声をMP3ファイルとしてダウンロード（ブラウザの標準ダウンロード機能を使用）
...
- **設定同期**: Chrome アカウント間での設定同期対応（APIキーは端末固有のため除く）
...
**制限事項**: 
- PDFファイルは現在対応していません（ChromeのPDFビューアー制限のため）
- iframe内のコンテンツには対応していません
